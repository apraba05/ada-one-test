...
## Stage 10 — Latency + transparency pass
- Measured before optimizing: local generation runs at **29.6 tok/s**, so latency is almost entirely tokens generated, not retrieval (0.2s) or model loading (1.7s warm-to-warm). That ruled out the things I'd have guessed at.
- **Default model 3B (llama3.2:3b), was 8B.** ~2x faster (≈6s vs ≈12s per answered question) and `eval.py` still passes 8/8 — the switch was validated, not assumed, which is exactly the decision the Stage 9 harness was built to make. Caveat logged: the 3B judge is less discriminating (flat 0.90 on bucket A where the 8B spreads 0.80-1.00), so `OLLAMA_MODEL=llama3.1:latest` remains the higher-fidelity path.
- Answer capped at ~120 words in the system prompt + `num_predict` 1024→400, judge 512→256. Typical answer fell from ~190 to ~43 words with no bucket change.
- Warm the LLM **and** the SentenceTransformer in a startup thread: the embedder is lazy-loaded on first search, so without it the first question paid both loads. First question went 19s → 6s. This was the demo's worst moment.
- `keep_alive: -1` on Ollama calls (a query makes two back-to-back calls; the default idle unload cost a reload between them), `num_ctx` 8192→4096 (largest prompt we build is ~1.6k).
- **Rejected streaming**, despite it being the obvious latency answer: the judge can refuse *after* generation, so streaming tokens would display an answer the gate is about to reject — it would break the product's one hard promise. Shipped honest pipeline-stage progress instead (client-side timing estimates, labelled as progress, claiming nothing about server state).
- **Margin report in eval.py**: 8/8 says nothing about robustness. Margins reveal the pricing case sits **+0.08** from reclassifying and is decided by `answers_question`, not by either numeric gate — i.e. the retrieval gate is not what protects Bucket B. Confirms the untested-band concern; `THIN` flags anything within 0.10.
- **Useful refusals**: judge-gate refusals now name the closest covered topics ("pricing isn't here, but Content ingestion / Knowledge setup are"). Deliberately NOT done for retrieval-gate refusals — nothing cleared the relevance floor there, so naming nearest docs would imply a connection the scores don't support.
- **Evidence panel**: API returns the top-3 retrieved chunk excerpts; the UI renders them under both answers and refusals so a claim can be audited in place rather than trusted.
- Debug note: a stale `next dev` server was serving HTML whose JS chunks 404'd, so the page rendered but never hydrated and was silently inert (Enter did nothing, no request fired). Cleared `.next` and restarted. Worth remembering — it looks exactly like a broken event handler.
